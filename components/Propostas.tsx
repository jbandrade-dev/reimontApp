...
export function Propostas({ propostas }: PropostasProps) {
  return (
    <section className="mt-16 " id="propostas">
      <main className="wrapper mob:px-4 pc:px-0 bg-red-500 mob:pb-0 pc:pb-6 rounded-md">
        <h2 className="sectionTitle text-white">O Brasil que queremos</h2>

        <ul className="grid pc:grid-cols-5 px-4 gap-2 mob:hidden pc:grid">
          {propostas.map((proposta) => {
            return <Proposta key={proposta.id} proposta={proposta} />;
          })}
        </ul>

        <article className="pc:hidden mob:flex">
          <Swiper
            slidesPerView={1}
            spaceBetween={16}
            freeMode={true}
